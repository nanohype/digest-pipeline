...
- **OpenTelemetry traces + metrics** exported OTLP to the cluster's OpenTelemetry Collector (`telemetry.monitoring.svc.cluster.local:4318`) → Tempo (traces) and Amazon Managed Prometheus (metrics). Pipeline phases and Bedrock sub-phases are explicit named spans.
- **Pino → stdout → Loki.** Log shipping is an infrastructure concern: apps emit structured JSON to stdout; the eks-gitops cluster OpenTelemetry Collector agent tails it into Loki (`{service="digest-pipeline-pipeline"}` / `digest-pipeline-api`). `trace_id` / `span_id` are auto-injected by `@opentelemetry/instrumentation-pino`, so every log record joins to Tempo.
...
- **Ingress + secrets + migrations.** `ingress.yaml` (ALB + ACM TLS: everything → web, the only public tier); `externalsecret.yaml` aggregates three AWS Secrets Manager entries into one Secret consumed via envFrom and composes `DATABASE_URL` via the External Secrets template engine; `migrate-job.yaml` is a Helm pre-install/pre-upgrade hook running `npm run migrate:up` before the new pods roll out.
- **Per-tenant AWS substrate** is declared in `spec.datastores`: the `main` Aurora Serverless v2 Postgres store and the voice-baseline (versioned, retained) + raw-aggregations (90-day lifecycle) S3 buckets, provisioned by the generic `tenant-substrate` component. SES send rides the `ses` capability. The operator generates the scoped IAM (datastore-access + capability-access) and binds the operator-owned `tenant-runtime` ServiceAccount to the tenant role via a Pod Identity association. The chart carries no IAM — no role, no policy, no role-arn annotation.
