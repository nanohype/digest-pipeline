...
- **`chart/`** — Helm chart with `pipeline-cronjob.yaml` (weekly Friday 09:00 UTC), `api-deployment.yaml` + `api-service.yaml` (Fastify on :3001), `web-deployment.yaml` + `web-service.yaml` (Next.js on :3000), `ingress.yaml` (cert-manager TLS; `/api/*` → api with rewrite-target, `/` → web), `externalsecret.yaml` aggregating three Secrets Manager entries, `migrate-job.yaml` Helm pre-install/pre-upgrade hook running schema migrations against Aurora. See [`chart/README.md`](chart/README.md) for the full template-by-template description.
...
Telemetry ships through the cluster-level Grafana Alloy collector installed by `eks-gitops` — no per-pod sidecars. Resource names, secret paths, and IAM grants are env-scoped (`digest-pipeline/staging/*` vs `digest-pipeline/production/*`). The staging IAM role **cannot** read production secrets and vice versa.
...
- Cluster addons (cert-manager, external-secrets, external-dns, the AWS Load Balancer Controller, Grafana Alloy — the OTLP receiver and log shipper — Tempo, Loki, and grafana-operator) → [`eks-gitops`](https://github.com/nanohype/eks-gitops). That catalog serves no `nginx` ingress class, which this chart's `Ingress` requests; see [`chart/README.md`](chart/README.md).
...
| `OTEL_EXPORTER_OTLP_ENDPOINT` / `OTEL_SERVICE_NAME` / `OTEL_RESOURCE_ATTRIBUTES` | chart env                                                                                              | OTLP target is the cluster Grafana Alloy collector at `alloy.monitoring.svc.cluster.local:4318` (no sidecar, no credential); tags traces with `service` + `agents.tenant`/`agents.platform` + `deployment.environment` |
...
The app ships no telemetry credentials. Alloy's OTLP receiver is unauthenticated in-cluster (reachable only through the chart's NetworkPolicy egress rule), and Alloy owns the upstream auth itself — SigV4 remote-write to Amazon Managed Prometheus signed with its own EKS Pod Identity, with Tempo and Loki both in-cluster. There is nothing for the app to hold.
