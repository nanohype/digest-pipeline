...
- **`chart/`** — Helm chart with `pipeline-cronjob.yaml` (weekly Friday 09:00 UTC), `api-deployment.yaml` + `api-service.yaml` (Fastify on :3001), `web-deployment.yaml` + `web-service.yaml` (Next.js on :3000), `ingress.yaml` (ingress-nginx + cert-manager: `/api/*` → api with rewrite-target, `/` → web), `externalsecret.yaml` aggregating three Secrets Manager entries, `migrate-job.yaml` Helm pre-install/pre-upgrade hook running schema migrations against Aurora. See [`chart/README.md`](chart/README.md) for the full template-by-template description.
