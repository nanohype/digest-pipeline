...
- **`platform.yaml`** — Platform CR (`platform.nanohype.dev/v1alpha1`) declaring dispatch as a tenant of the `protohype` team on the `eks-agent-platform` operator. Operator reconciles Namespace, ResourceQuota, IRSA role, KMS grants, S3 bucket policy.
