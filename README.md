...
- **`src/runtime/`** — vendored `@nanohype/runtime` modules (`resilience.ts`, `registry.ts`, `pii.ts`, `workos-directory.ts`): byte-identical copies of `nanohype/library/runtime/src`, the same consumption model as the vendored `tenant-chart-base` chart. `npm run sync:runtime` re-copies from the source of truth; CI runs `npm run sync:runtime:check` as a drift gate. `withTimeout` + `withRetry` from here bound every external call site; `TimeoutError` is a distinct type so callers can branch on it.
...
- Provider registry pattern (`createRegistry<T>` from `src/runtime/registry.ts`) for aggregators and identity resolvers
- Resilience contract: every external call uses `withTimeout` (8s default, 15s for Slack history) + `withRetry(3, jitter)`
- Vendored runtime primitives (`src/runtime/`) are never edited in place — change `nanohype/library/runtime` and re-sync (`npm run sync:runtime`)
