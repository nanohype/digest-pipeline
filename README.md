...
| `MODEL_GATEWAY_ENDPOINT`                                                         | chart env                                                                                              | The Platform's ModelGateway (`ModelGateway.status.endpoint`). The app holds no AWS model credential — the gateway signs for Bedrock with its own Pod Identity                                                                              |
| `MODEL_ROUTE`                                                                    | chart env                                                                                              | A route on that gateway (default `default`), not a Bedrock model id. The `ModelGateway` CR maps the route to a concrete inference profile, so the `us.` prefix the Claude family requires lives in one CR rather than every tenant's values |
