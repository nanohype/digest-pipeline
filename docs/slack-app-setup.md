...
DigestPipeline needs a Slack bot user that can (a) read recent messages from the announcements + team channels as ingestion sources and (b) post "Draft ready" notifications + operational alerts to the newsletter-review channel. This doc walks through provisioning the app once per environment.

> Different Slack workspace per environment. Staging and production should not share a bot. The review channel, channels the bot reads from, and HR-bot user IDs are all env-scoped fields on `digest-pipeline/{env}/slack` and `digest-pipeline/{env}/runtime-config`.

## 1. Create the app

1. Go to [api.slack.com/apps](https://api.slack.com/apps) → **Create New App** → **From scratch**.
2. Name it `digest-pipeline-staging` (or `digest-pipeline-production`). Pick the target workspace.
3. Basic Information → record the **Signing Secret** (unused for inbound traffic today — DigestPipeline does not receive Slack events — but keep it in case you later add a slash command).
...
That's it — no write access beyond `chat:write`, no admin scopes, no private-channel scopes. DigestPipeline never reads DMs or private channels.

Click **Install to Workspace** at the top of the same page. Record the **Bot User OAuth Token** (`xoxb-…`) — it goes into `digest-pipeline/{env}/slack` as `botToken`.
...
|---|---|---|
| `#announcements` (or your team's equivalent) | Company-wide announcements, wins, recognitions. The aggregator reads the last 7 days; items ≥20 chars (`MIN_ANNOUNCEMENT_LENGTH`) and ≤2000 chars (`MAX_TOKEN_LENGTH`) pass through. | `digest-pipeline/{env}/slack.announcementsChannelId` |
| `#team` (or your cross-team updates channel) | Weekly team updates, standup summaries, shout-outs. Same filters. | `digest-pipeline/{env}/slack.teamChannelId` |
| `#newsletter-review` | Where DigestPipeline posts "Draft ready" and pipeline alerts. The Chief of Staff + backup approvers should be members. | `digest-pipeline/{env}/runtime-config.slackReviewChannelId` |
...
```
/invite @digest-pipeline-staging   # or @digest-pipeline-production
```
...
3. Add the IDs to `digest-pipeline/{env}/slack.hrBotUserIds`:
...
`auth.test` should return `{ "ok": true, "user": "digest-pipeline-staging", ... }`. `conversations.members` should include your bot's user ID (visible as `auth.test.user_id`).
...
- **No slash commands.** DigestPipeline posts notifications and reads channel history; it never handles inbound Slack events. No request-URL endpoint, no Socket Mode connection.
...
If a future iteration adds a `/digest-pipeline` slash command for operators (e.g. triggering a manual run), that's a new scope + a new Slack event ingress — document it in this file when it lands.
