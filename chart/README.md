...
- `templates/`
  ...
  - `ingress.yaml` — routes `/api/*` → api Service (with rewrite-target to strip `/api`), `/` → web Service; cert-manager TLS. `ingress.className` defaults to `nginx`, and the rewrite annotation is nginx-specific
  ...
  - `networkpolicy.yaml` — ingress: the `ingress-nginx` namespace → api/web + intra-pod web → api; egress: DNS + HTTPS + Postgres on cluster VPC CIDR
