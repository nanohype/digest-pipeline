...
npm run platform:validate            # gate platform.yaml against the vendored CRD schemas
npm run platform:validate:self-test  # prove the gate rejects what it claims to catch
npm run schemas:crd                  # re-vendor schemas/crd/ + re-record the digests
npm run schemas:crd:check            # CI gate: copies match eks-agent-platform at the pinned commit
