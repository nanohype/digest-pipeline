...
The secret `digest-pipeline/${env}/grafana-cloud` carries `{ instanceId, apiToken, otlpEndpoint, authHeader }` for the cluster collector's upstream auth. Operator pre-computes `authHeader = "Basic " + base64("instanceId:apiToken")` once. (No `lokiEndpoint` field — logs don't go through the collector.)
